...
sql.query = {
    ...
    filter_advertised_trips: "SELECT DISTINCT A.driver_username, A.s_time, A.e_time, A.s_date, A.e_date, A.license_plate, A.s_location, A.e_location, A.min_bid FROM advertised_trips A WHERE NOT A.driver_username=$1 AND s_location=$2 EXCEPT SELECT DISTINCT B.driver_username, B.s_time, B.e_time, B.s_date, B.e_date, B.license_plate, B.s_location, B.e_location, B.min_bid FROM bids B WHERE (B.is_win='t') OR (B.passenger_username=$1 AND B.is_win='f')",
    //trigger
